...
for f, line in pairs:
    for net_type in network_types:
        ...
        num_models += 1
        if num_models % SPEC_SIZE == 0:
            with open(f"./spec_files_{SPEC_SIZE}_PS/specs_{num_models // SPEC_SIZE}.txt", "w") as out_file:
                json.dump(specs, out_file)
            specs = []

if len(specs) > 0:
    with open(f"./spec_files_{SPEC_SIZE}_PS/specs_{(num_models // SPEC_SIZE)+1}.txt", "w") as out_file:
        json.dump(specs, out_file)
